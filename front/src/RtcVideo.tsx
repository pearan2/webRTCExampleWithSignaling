    ...
    const animationCallBack = () => {
      ...
      this.peers.forEach((peer) => {
        if (peer.dc.readyState === "open")
          peer.dc.send(JSON.stringify(this.me));
        peer.draw(this.drawContext);

        if (peer.connectedAudioElement) {
          const distance = Math.sqrt(
            Math.pow(this.me.centerPos.x - peer.centerPos.x, 2) +
              Math.pow(this.me.centerPos.y - peer.centerPos.y, 2)
          );
...
const App = () => {
  ...
  const setConnectedAudioElementByID = (
